...
def orca(graph):
    ...
    output = sp.check_output(
        [
            str(
                os.path.join(
                    os.path.dirname(os.path.realpath(__file__)), "./orca/orca"
                )
            ),
            "node",
            "4",
...
def eval_acc_tree_graph(G_list):
    ...
    for gg in G_list:
        if nx.is_tree(gg):
            count += 1
    return count / float(len(G_list)), count
...
def eval_fraction_unique_non_isomorphic_valid(
        fake_graphs, train_graphs, count_valid
):
    count_isomorphic = 0
    count_non_unique = 0
    fake_evaluated = []
...
class SpectreSamplingMetrics(nn.Module):
    def __init__(self, dataloaders, compute_emd, metrics_list, test=False):
        super().__init__()
        ...
        self.reference_graphs = self.test_graphs if test else self.val_graphs
    ...
    def compute_train_ref_mmd(self):
        if "degree" in self.metrics_list:
            ...
            self.train_degree = degree_stats(
        ...
        if "orbit" in self.metrics_list:
            print("Pre-computing orbit stats...")
            self.train_orbit = orbit_stats_all(
                self.reference_graphs,
                self.train_graphs,
                compute_emd=self.compute_emd
            )
    ...
    def forward(self, generated_graphs: list):
        ...
        for i, adj in enumerate(generated_graphs):
            A = adj.bool().cpu().numpy()
            adjacency_matrices.append(A)

            nx_graph = nx.from_numpy_array(A)
            networkx_graphs.append(nx_graph)
            generated_densities.append(density(nx_graph))

        gt_densities = []
        for test_graph in self.test_graphs:
            gt_densities.append(density(test_graph))

        print(f'Generated densities: {sum(generated_densities)/len(generated_densities)}')
        print(f'Ground truth densities: {sum(gt_densities)/len(gt_densities)}')
        ...
        if "degree" in self.metrics_list:
            ...
            if "ratio" in self.metrics_list:
                ratios["degree"] = degree / self.train_degree
        ...
        if "spectre" in self.metrics_list:
            ...
            if "ratio" in self.metrics_list:
                ratios["spectre"] = spectre / self.train_spectre
            if wandb.run is not None:
        ...
        if "clustering" in self.metrics_list:
            ...
            if "ratio" in self.metrics_list:
                ratios["clustering"] = clustering / self.train_clustering
            if wandb.run is not None:
        ...
        if "orbit" in self.metrics_list:
            ...
            orbit = orbit_stats_all(
                self.reference_graphs, networkx_graphs, compute_emd=self.compute_emd
            )
            to_log["orbit"] = orbit
            if "ratio" in self.metrics_list:
                ratios["orbit"] = orbit / self.train_orbit
            if wandb.run is not None:
                wandb.run.summary["orbit"] = orbit

        if "sbm" in self.metrics_list:
            print("Computing accuracy...")
            sbm_acc, count = eval_acc_sbm_graph(networkx_graphs, refinement_steps=100, strict=True)
            to_log["acc"] = sbm_acc
            print(f'SBM accuracy: {sbm_acc}')
            if wandb.run is not None:
                wandb.run.summary["acc"] = sbm_acc

        if "planar" in self.metrics_list:
            print("Computing planar accuracy...")
            planar_acc, count = eval_acc_planar_graph(networkx_graphs)
            print(f'Planar accuracy: {planar_acc}')
            to_log["acc"] = planar_acc
            if wandb.run is not None:
                wandb.run.summary["acc"] = planar_acc

        if "tree" in self.metrics_list:
            print("Computing tree accuracy...")
            tree_acc, count = eval_acc_tree_graph(networkx_graphs)
            print(f'Tree accuracy: {tree_acc}')
            to_log["acc"] = tree_acc
            if wandb.run:
                wandb.run.summary["acc"] = tree_acc

        # if "planar" in self.metrics_list or "sbm" in self.metrics_list or "tree" in self.metrics_list:
        ...
        if "ratio" in self.metrics_list:
            to_log['ratio'] = sum(list(ratios.values())) / len(ratios)
...
class PlanarSamplingMetrics(SpectreSamplingMetrics):
    def __init__(self, dataloaders, test=False):
        super().__init__(
