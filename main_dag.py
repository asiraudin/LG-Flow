...
from models import AutoencoderKL, EncodermLPE, DecoderDirected
...
from evaluation.synthetic import TPUSamplingMetrics, PriceSamplingMetrics


def evaluate(interpolant, model, autoencoder, scale_factor, q, node_distribution, sampling_metrics, cfg, step, device_id):
    sample_graphs = []
    sampling_start = time.time()
    for _ in range(cfg.num_sample_batch):
        n_nodes = node_distribution.sample_n(cfg.num_samples, device=device_id)  # (bs)
...
@hydra.main(version_base=None, config_path="./configs", config_name="tputile_fm_train")
def main(cfg):
    ...
    if master_process:
        if cfg.dataset.name == 'tpu_tile':
            sampling_metrics = TPUSamplingMetrics({'train': train_loader, 'val': val_loader, 'test': test_loader})
        elif cfg.dataset.name == 'price':
            sampling_metrics = PriceSamplingMetrics({'train': train_loader, 'val': val_loader, 'test': test_loader})
        else:
    ...
    autoencoder = AutoencoderKL(encoder, decoder, True).to(device_id)
    autoencoder.eval()
    load_checkpoint(
    ...
    model = DiT(num_layers=cfg.denoiser.num_layers, num_heads=cfg.denoiser.num_heads, in_dim=cfg.encoder.pe_dim,
                embed_dim=cfg.denoiser.embed_dim, dropout=cfg.dropout).to(device_id)
        ...
        for batch in train_loader:
            ...
            if step % int(cfg.val_after) == 0 and step >= int(cfg.val_after):
                ...
                if master_process:
                    ...
                    eval_module = ema_model.ema_model
                    eval_module.eval()
                    ...
                    target_metric = 'sampling/frac_unic_non_iso_valid'
                    if best_val is None or metrics[target_metric] > best_val:
                    ...
                    if cfg.checkpoint is not None:
                        ...
                        save_path = str(p.with_name(f"{p.stem}{f'step_{step}'}{p.suffix}"))
                        save_checkpoint(
                            save_path,
                            step,
                            module,
